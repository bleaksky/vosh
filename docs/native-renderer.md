...
- **M6 — cross-platform. CODE COMPLETE, hardware passes pending.** The
  renderer core (grid, cell renderer, surface state, pointer logic)
  compiles clippy-clean on all three targets, gated by the
  `native_surface` cfg emitted from build.rs. Platform glue:
  `native_surface/macos.rs` (NSView + CAMetalLayer + full mouse, verified
  live), `native_surface/windows.rs` (child HWND + WndProc with full mouse
  - D3D12), and `native_surface/linux.rs` (raw X11 child over the GTK
    toplevel + Vulkan, display-only: input propagates through to the
    webview; Wayland detects and falls back to xterm). Glyph rasterization
    off macOS goes through font-kit's platform library
    (DirectWrite/FreeType). Windows and Linux are compile-verified only and
    need hardware passes before the frontend enables them.
...
- **Linux hardware pass.** The Linux surface (`native_surface/linux.rs`)
  ...
  count keeps rustc from being OOM-killed in the container). Untested on
  real hardware: X11 child z-order over the webkit2gtk widget, HiDPI
  scale, and Vulkan present need eyes. Display-only for now — the child
  selects no X events so input falls through to the webview; wheel and
  selection would come from DOM forwarding when someone enables it.
  Wayland detects at install and falls back to xterm.
