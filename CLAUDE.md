...
Phase 6 complete. Each session runs a tick timer (default 30 seconds, configurable via `#tick interval <secs>`) that emits `session://tick` every 250 ms with the remaining time and whether this tick is the fire moment. The status pane shows the countdown at the top of the side panel, flashes red below 5 seconds, and beeps via Web Audio when the timer fires (toggle with `#tick sound on|off`). The timer can reset on a regex match against incoming output (`#tick on {pattern}`) and run an auto-fire command through the input pipeline at each fire (`#tick fire <command>`). The full slash family is `#tick`, `#tick interval`, `#tick reset`, `#tick on`, `#tick off`, `#tick fire`, `#tick nofire`, `#tick sound`, `#tick disable`, `#tick enable`.
