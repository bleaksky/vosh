...
Phase 4 complete. mudclient-gmcp parses GMCP package + JSON payloads. The negotiator now accepts WILL GMCP and DO GMCP. On agreement the session sends Core.Hello (with client name and version) plus Core.Supports.Set for the Char, Room, and Comm packages. Incoming GMCP messages flow through gmcp_bind which writes known packages onto session-scoped variables (Char.Vitals goes to hp, mp, sp, etc.; Room.Info goes to room_name, room_area, room_id; Char.Name and Char.Status get the char_ prefix). Every GMCP message also fires a session://gmcp Tauri event so future panels and the script engine can subscribe. Phase 5 (split panes and chat capture) starts only after explicit approval in chat.
