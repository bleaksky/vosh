...
Phase 8 complete. Embedded Lua via `mlua` (Lua 5.4, vendored) ships in `crates/script`. A `ScriptEngine` lives in the Profile and exposes a `mud.*` API: `send`, `input`, `echo`, `log`, `alias`/`unalias`, `var`/`set_var`/`set_profile_var`/`unset_var`, `trigger`/`untrigger` (regex with Lua callback), `on_gmcp` (per-package subscribers), `timer`/`cancel_timer` (one-shot via tokio interval). Lua callbacks queue side effects as `Action` values that the session loop applies under the existing profile lock; the script engine is sandboxed by default (`os.execute`, `io`, `loadfile`, `dofile`, `load`, `require`, `package` removed). On every line and every GMCP push the session refreshes a variable snapshot (so `mud.var` reads fresh), dispatches Lua triggers and subscribers, and applies the queued actions. Slash commands `#script load <name>` (reads `<app_data_dir>/scripts/<name>.lua`), `#script reload`, `#scripts`, `#lua <code>`. Demo script lives at `scripts/combat.lua` and covers auto-loot, low-HP flee, combat alias, and a tell reminder timer.
