...
Phase 9 complete. `ProfileConfig` is the serde-friendly snapshot that survives across launches (connection defaults, aliases, profile-scoped vars, triggers, tick config, autoload script names). Saved at `<app_data_dir>/profile.toml` and auto-loaded on startup. Slash commands `#profile save`, `#profile load`, `#profile reset`. Tauri commands `profile_export` and `profile_import` back the new Settings drawer that ships a live TOML editor with apply, copy, paste, download, upload buttons, mirroring the Triggers drawer. The `tintin_import` module parses `#alias` and `#variable` directives out of TinTin++ `.tin` files (with brace-nesting support); `#import-tintin <path>` loads them into the live profile and reports unsupported directives by count. Aliases gain serde derives so they round-trip through TOML cleanly.
