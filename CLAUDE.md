...
Phase 5 complete. The middle area now flexes to fit the terminal, an optional side panel (status on top, chat on bottom), and the triggers drawer when open. The status pane subscribes to `session://gmcp` and renders char name, level, class, HP/MP/SP bars, and current room. The chat pane consumes `Comm.Channel.Text` GMCP messages plus any line a trigger sends to it via the `route` action (which now emits a `session://routed` Tauri event). Side panel visibility persists in `localStorage`; per-profile persistence on disk lands in Phase 9.
