...
Phase 3 complete. The trigger engine matches incoming MUD lines against user-defined regex patterns and supports five actions: highlight (wraps matched text with SGR), gag (drops the line), replace (substitutes with capture interpolation), send (queues a command back to the server), and route (Phase 5 will deliver to a named pane). Triggers compose by priority. The session loop now buffers bytes through a LineAccumulator before feeding the engine. Slash commands `#trigger`, `#untrigger`, `#triggers` join the family. A Triggers drawer in the UI loads, edits, and saves the trigger list as JSON, with copy, paste, download, and upload buttons for import and export. Phase 4 (GMCP and event bus) starts only after explicit approval in chat.
