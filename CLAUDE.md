...
Phase 6 complete. Each session runs a tick timer (default 30 seconds, configurable via `#tick interval <secs>`) that emits `session://tick` every 250 ms. The status pane shows the countdown at the top of the side panel, flashes red below 5 seconds, and beeps via Web Audio when the timer fires (toggle with `#tick sound on|off`). The timer auto-resets on incoming GMCP `World.Time` packages whenever the `hour` field advances; ROM derivatives like Aabahran fire their tick exactly at that moment, so no manual configuration is needed there. For MUDs that do not push `World.Time`, set a regex with `#tick on {pattern}`. The auto-fire command runs through the input pipeline at each fire (`#tick fire <command>`). Full slash family: `#tick`, `#tick interval`, `#tick reset`, `#tick on`, `#tick off`, `#tick fire`, `#tick nofire`, `#tick sound`, `#tick disable`, `#tick enable`. World now joins Char/Room/Comm in the Core.Supports.Set request.
