...
export const HELP_TOPICS: HelpTopic[] = [
  ...
  },
  {
    ...
    section: 'Shape the window',
    body: "The vitals readout shows hp, mana, and moves. Configure it in the `vitals` tab in Settings, where changes save automatically and you can drag the live preview's bars to scrub the numbers.\n\n- Pick a layout. The options are `ledger`, `gauges`, `pips`, `strip`, `stacked`, and `inline`. The first four share a card with a `vitals` head and the tick countdown beside it, and under 20% a vital turns red and pulses until it climbs back to 25%.\n- Ledger draws one column per vital. Each column shows the current value as a large mono figure with its max beside it, the change since the last tick, and a thin filament that carries the percent. The columns stack into rows when the card is narrower than 300px.\n- Gauges draws one filled track per vital with the label, the change, and the value inside it.\n- Pips draws ten cells per vital, filled or hollow, with a partial cell for the remainder.\n- Strip drops the card. The vitals ride the status bar under the input row as segments, the panel zone stays empty, and the combat target and tick join as segments of their own.\n- Outside those four, choose columns with the `bar`, `percent`, `numeric`, and `delta` pills, set `bar style` to `solid`, `ramped`, or `track`, and pick bar glyphs and width.\n- Open the `advanced` disclosure to recolor hp, mana, and moves, or turn on `drain through red as bars empty`.\n- Tick `pulse red vignette under 30% hp` to pulse a red vignette when hp drops under 30%.\n- Turn on `custom template (overrides layout)` to write the readout yourself with tokens like `%hp`, `%pct_hp`, `%bar_hp`, `%tick`, and `%time`. Any `Char.Vitals` or `Char.Worth` field resolves as `%fieldname`, and `%%` prints a literal percent.\n\nSet `strip size` to cap the block width when the bar sits in a top or bottom row, and pick `left`, `center`, or `right` to place it. A size of 0 fills the row. The setting does nothing in a side column, where the column width sizes the bar, and the strip layout ignores it since it rides the status bar.\n\nPlace the bar in any zone from the `panels` tab. It ships in the right column, listed as `vitals (hp bar)`. Tracked affects also live in the `panels` tab, not here.\n\n`reset vitals` restores the stock config.",
  },
  {
